/*
 * Vulkan Samples - edited for the GraphicsFuzz vulkan worker
 *
...
VkResult init_global_extension_properties(layer_properties &layer_props) {
    ...
    do {
        res = VKCALL(vkEnumerateInstanceExtensionProperties(layer_name, &instance_extension_count, NULL));
        if (res) return res;
        ...
        res = VKCALL(vkEnumerateInstanceExtensionProperties(layer_name, &instance_extension_count, instance_extensions));
...
VkResult init_global_layer_properties(struct sample_info &info) {
     ...
     */
    do {
        res = VKCALL(vkEnumerateInstanceLayerProperties(&instance_layer_count, NULL));
        if (res) return res;
        ...
        res = VKCALL(vkEnumerateInstanceLayerProperties(&instance_layer_count, vk_props));
    } while (res == VK_INCOMPLETE);
...
VkResult init_device_extension_properties(struct sample_info &info, layer_properties &layer_props) {
    ...
    do {
        res = VKCALL(vkEnumerateDeviceExtensionProperties(info.gpus[0], layer_name, &device_extension_count, NULL));
        if (res) return res;
        ...
        res = VKCALL(vkEnumerateDeviceExtensionProperties(info.gpus[0], layer_name, &device_extension_count, device_extensions));
    } while (res == VK_INCOMPLETE);
...
VkResult init_instance(struct sample_info &info, char const *const app_short_name) {
    ...
    VkResult res = VKCALL(vkCreateInstance(&inst_info, NULL, &info.inst));
...
VkResult init_device(struct sample_info &info) {
    ...
    res = VKCALL(vkCreateDevice(info.gpus[0], &device_info, NULL, &info.device));
    assert(res == VK_SUCCESS);
...
VkResult init_enumerate_device(struct sample_info &info, uint32_t gpu_count) {
    ...
    VkResult res = VKCALL(vkEnumeratePhysicalDevices(info.inst, &gpu_count, NULL));
    assert(gpu_count);
    info.gpus.resize(gpu_count);

    res = VKCALL(vkEnumeratePhysicalDevices(info.inst, &gpu_count, info.gpus.data()));
...
void init_depth_buffer(struct sample_info &info) {
    ...
    /* Create image */
    res = VKCALL(vkCreateImage(info.device, &image_info, NULL, &info.depth.image));
    assert(res == VK_SUCCESS);
    ...
    /* Allocate memory */
    res = VKCALL(vkAllocateMemory(info.device, &mem_alloc, NULL, &info.depth.mem));
    assert(res == VK_SUCCESS);

    /* Bind memory */
    res = VKCALL(vkBindImageMemory(info.device, info.depth.image, info.depth.mem, 0));
    assert(res == VK_SUCCESS);

    /* Create image view */
    view_info.image = info.depth.image;
    res = VKCALL(vkCreateImageView(info.device, &view_info, NULL, &info.depth.view));
    assert(res == VK_SUCCESS);
}
...
// Construct the surface description:
    ...
    res = VKCALL(vkCreateWin32SurfaceKHR(info.inst, &createInfo, NULL, &info.surface));
#elif defined(__ANDROID__)
    ...
    res = VKCALL(vkCreateIOSSurfaceMVK(info.inst, &createInfo, NULL, &info.surface));
#elif defined(VK_USE_PLATFORM_MACOS_MVK)
    ...
    createInfo.flags = 0;
    createInfo.pView = info.window;
    res = VKCALL(vkCreateMacOSSurfaceMVK(info.inst, &createInfo, NULL, &info.surface));
#else
    ...
    res = VKCALL(vkCreateXcbSurfaceKHR(info.inst, &createInfo, NULL, &info.surface));
    ...
    res = VKCALL(vkGetPhysicalDeviceSurfaceFormatsKHR(info.gpus[0], info.surface, &formatCount, NULL));
    assert(res == VK_SUCCESS);
    VkSurfaceFormatKHR *surfFormats = (VkSurfaceFormatKHR *)malloc(formatCount * sizeof(VkSurfaceFormatKHR));
    res = VKCALL(vkGetPhysicalDeviceSurfaceFormatsKHR(info.gpus[0], info.surface, &formatCount, surfFormats));
    assert(res == VK_SUCCESS);
...
void init_presentable_image(struct sample_info &info) {
    ...
    res = VKCALL(vkCreateSemaphore(info.device, &imageAcquiredSemaphoreCreateInfo, NULL, &info.imageAcquiredSemaphore));
    assert(!res);

    // Get the index of the next available swapchain image:
    res = VKCALL(vkAcquireNextImageKHR(info.device, info.swap_chain, UINT64_MAX, info.imageAcquiredSemaphore, VK_NULL_HANDLE,
                                &info.current_buffer));
...
void execute_queue_cmdbuf(struct sample_info &info, const VkCommandBuffer *cmd_bufs, VkFence &fence) {
    ...
    res = VKCALL(vkQueueSubmit(info.graphics_queue, 1, submit_info, fence));
    assert(!res);
}
...
void init_swap_chain(struct sample_info &info, VkImageUsageFlags usageFlags) {
    ...
    res = VKCALL(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(info.gpus[0], info.surface, &surfCapabilities));
    assert(res == VK_SUCCESS);

    uint32_t presentModeCount;
    res = VKCALL(vkGetPhysicalDeviceSurfacePresentModesKHR(info.gpus[0], info.surface, &presentModeCount, NULL));
    assert(res == VK_SUCCESS);
    VkPresentModeKHR *presentModes = (VkPresentModeKHR *)malloc(presentModeCount * sizeof(VkPresentModeKHR));
    assert(presentModes);
    res = VKCALL(vkGetPhysicalDeviceSurfacePresentModesKHR(info.gpus[0], info.surface, &presentModeCount, presentModes));
    assert(res == VK_SUCCESS);
    ...
    res = VKCALL(vkCreateSwapchainKHR(info.device, &swapchain_ci, NULL, &info.swap_chain));
    assert(res == VK_SUCCESS);

    res = VKCALL(vkGetSwapchainImagesKHR(info.device, info.swap_chain, &info.swapchainImageCount, NULL));
    assert(res == VK_SUCCESS);

    VkImage *swapchainImages = (VkImage *)malloc(info.swapchainImageCount * sizeof(VkImage));
    assert(swapchainImages);
    res = VKCALL(vkGetSwapchainImagesKHR(info.device, info.swap_chain, &info.swapchainImageCount, swapchainImages));
    assert(res == VK_SUCCESS);
    ...
    for (uint32_t i = 0; i < info.swapchainImageCount; i++) {
        ...
        res = VKCALL(vkCreateImageView(info.device, &color_image_view, NULL, &sc_buffer.view));
...
void init_uniform_buffer(struct sample_info &info) {
    ...
    // Init uniforms
    info.uniform_datas.resize(info.sample_uniforms.size());
    ...
    for (int i = 0; i < info.sample_uniforms.size(); i++) {

        buf_info.size = info.sample_uniforms[i].size;
        res = VKCALL(vkCreateBuffer(info.device, &buf_info, NULL, &info.uniform_datas[i].buf));
        assert(res == VK_SUCCESS);

        vkGetBufferMemoryRequirements(info.device, info.uniform_datas[i].buf, &mem_reqs);
        ...
        res = VKCALL(vkAllocateMemory(info.device, &alloc_info, NULL, &(info.uniform_datas[i].mem)));
        assert(res == VK_SUCCESS);

        res = VKCALL(vkMapMemory(info.device, info.uniform_datas[i].mem, 0, mem_reqs.size, 0,
                          (void **) &pData));
        assert(res == VK_SUCCESS);

        memcpy(pData, info.sample_uniforms[i].pval, info.sample_uniforms[i].size);
        //delete(info.sample_uniforms[i].pval);

        vkUnmapMemory(info.device, info.uniform_datas[i].mem);

        res = VKCALL(vkBindBufferMemory(info.device, info.uniform_datas[i].buf, info.uniform_datas[i].mem,
                                 0));
        assert(res == VK_SUCCESS);

        info.uniform_datas[i].buffer_info.buffer = info.uniform_datas[i].buf;
        info.uniform_datas[i].buffer_info.offset = 0;
        info.uniform_datas[i].buffer_info.range = info.sample_uniforms[i].size;

    }
}

void init_descriptor_and_pipeline_layouts(struct sample_info &info, bool use_texture) {
    VkDescriptorSetLayoutBinding *layout_bindings = new VkDescriptorSetLayoutBinding[info.sample_uniforms.size()];
    ...
    descriptor_layout.bindingCount = info.sample_uniforms.size(); //use_texture ? 2 : 1;
    descriptor_layout.pBindings = layout_bindings;

    VkResult U_ASSERT_ONLY res;

    info.desc_layout.resize(NUM_DESCRIPTOR_SETS);
    res = VKCALL(vkCreateDescriptorSetLayout(info.device, &descriptor_layout, NULL, info.desc_layout.data()));
    assert(res == VK_SUCCESS);
    ...
    res = VKCALL(vkCreatePipelineLayout(info.device, &pPipelineLayoutCreateInfo, NULL, &info.pipeline_layout));
    assert(res == VK_SUCCESS);
...
void init_renderpass(struct sample_info &info, bool include_depth, bool clear, VkImageLayout finalLayout) {
    ...
    res = VKCALL(vkCreateRenderPass(info.device, &rp_info, NULL, &info.render_pass));
    assert(res == VK_SUCCESS);
}
...
void init_framebuffers(struct sample_info &info, bool include_depth) {
    ...
    for (i = 0; i < info.swapchainImageCount; i++) {
        ...
        res = VKCALL(vkCreateFramebuffer(info.device, &fb_info, NULL, &info.framebuffers[i]));
        assert(res == VK_SUCCESS);
    }
}
void init_command_pool(struct sample_info &info) {
    ...
    res = VKCALL(vkCreateCommandPool(info.device, &cmd_pool_info, NULL, &info.cmd_pool));
    assert(res == VK_SUCCESS);
}
...
void init_command_buffer(struct sample_info &info) {
    ...
    res = VKCALL(vkAllocateCommandBuffers(info.device, &cmd, &info.cmd));
    assert(res == VK_SUCCESS);
}
void execute_begin_command_buffer(struct sample_info &info) {
    ...
    res = VKCALL(vkBeginCommandBuffer(info.cmd, &cmd_buf_info));
    assert(res == VK_SUCCESS);
}
...
void init_vertex_buffer(struct sample_info &info, const void *vertexData, uint32_t dataSize, uint32_t dataStride,
    ...
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    res = VKCALL(vkCreateBuffer(info.device, &buf_info, NULL, &info.vertex_buffer.buf));
    assert(res == VK_SUCCESS);
    ...
    res = VKCALL(vkAllocateMemory(info.device, &alloc_info, NULL, &(info.vertex_buffer.mem)));
    assert(res == VK_SUCCESS);
    info.vertex_buffer.buffer_info.range = mem_reqs.size;
    info.vertex_buffer.buffer_info.offset = 0;

    uint8_t *pData;
    res = VKCALL(vkMapMemory(info.device, info.vertex_buffer.mem, 0, mem_reqs.size, 0, (void **)&pData));
    assert(res == VK_SUCCESS);
    ...
    res = VKCALL(vkBindBufferMemory(info.device, info.vertex_buffer.buf, info.vertex_buffer.mem, 0));
    assert(res == VK_SUCCESS);
...
void init_descriptor_pool(struct sample_info &info, bool use_texture) {
    ...
    VkResult U_ASSERT_ONLY res;
    VkDescriptorPoolSize *type_count = new VkDescriptorPoolSize[info.sample_uniforms.size()];
    ...
    descriptor_pool.poolSizeCount = info.sample_uniforms.size();
    descriptor_pool.pPoolSizes = type_count;

    res = VKCALL(vkCreateDescriptorPool(info.device, &descriptor_pool, NULL, &info.desc_pool));
    assert(res == VK_SUCCESS);
...
void init_descriptor_set(struct sample_info &info, bool use_texture) {
    ...
    res = VKCALL(vkAllocateDescriptorSets(info.device, alloc_info, info.desc_set.data()));
    assert(res == VK_SUCCESS);

    VkWriteDescriptorSet *writes = new VkWriteDescriptorSet[info.sample_uniforms.size()];

    for (int i = 0; i < info.sample_uniforms.size(); i++) {
        writes[i] = {};
        writes[i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        writes[i].pNext = NULL;
        writes[i].dstSet = info.desc_set[0];
        writes[i].descriptorCount = 1;
        writes[i].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        writes[i].dstArrayElement = 0;

        writes[i].pBufferInfo = &info.uniform_datas[i].buffer_info;
        writes[i].dstBinding = i;
    }

    vkUpdateDescriptorSets(info.device, info.sample_uniforms.size(), writes, 0, NULL);
    delete [] writes;
}

void loadSpv(std::ifstream& ifs, std::vector<unsigned int>& spv) {
    unsigned int i;
    while (!ifs.eof()) {
        ifs.read((char *)(&i), sizeof(unsigned int));
        if (ifs.gcount() == sizeof(unsigned int)) {
            spv.push_back(i);
        } else {
            assert(ifs.eof() && "Error when loading SPV binary");
        }
    }
}
...
void init_shaders(struct sample_info &info, const char *vertShaderText, const char *fragShaderText) {
    ...
    if (vertShaderText) {
        ...
        // Hugues: use pre-generated SPV binary, if present
        std::ifstream vertFile("/sdcard/graphicsfuzz/test.vert.spv");
        if (vertFile.fail()) {
            vklog("Using vertex shader source: GLSL\n");
            retVal = GLSLtoSPV(VK_SHADER_STAGE_VERTEX_BIT, vertShaderText, vtx_spv);
            assert(retVal);
        } else {
            vklog("Using vertex shader source: SPV\n");
            loadSpv(vertFile, vtx_spv);
        }
        ...
        res = VKCALL(vkCreateShaderModule(info.device, &moduleCreateInfo, NULL, &info.shaderStages[0].module));
        assert(res == VK_SUCCESS);
    }
    ...
    if (fragShaderText) {
        ...
        // Hugues: use pre-generated SPV binary, if present
        std::ifstream fragFile("/sdcard/graphicsfuzz/test.frag.spv");
        if (fragFile.fail()) {
            vklog("Using fragment shader source: GLSL\n");
            retVal = GLSLtoSPV(VK_SHADER_STAGE_FRAGMENT_BIT, fragShaderText, frag_spv);
            assert(retVal);
        } else {
            vklog("Using fragment shader source: SPV\n");
            loadSpv(fragFile, frag_spv);
        }
        ...
        res = VKCALL(vkCreateShaderModule(info.device, &moduleCreateInfo, NULL, &info.shaderStages[1].module));
        assert(res == VK_SUCCESS);
    }
...
void init_pipeline_cache(struct sample_info &info) {
    ...
    res = VKCALL(vkCreatePipelineCache(info.device, &pipelineCache, NULL, &info.pipelineCache));
    assert(res == VK_SUCCESS);
}
...
void init_pipeline(struct sample_info &info, VkBool32 include_depth, VkBool32 include_vi) {
    ...
    res = VKCALL(vkCreateGraphicsPipelines(info.device, info.pipelineCache, 1, &pipeline, NULL, &info.pipeline));
    assert(res == VK_SUCCESS);
}
...
void init_sampler(struct sample_info &info, VkSampler &sampler) {
    ...
    /* create sampler */
    res = VKCALL(vkCreateSampler(info.device, &samplerCreateInfo, NULL, &sampler));
    assert(res == VK_SUCCESS);
}
...
void init_image(struct sample_info &info, texture_object &texObj, const char *textureName, VkImageUsageFlags extraUsages,
    ...
    res = VKCALL(vkCreateImage(info.device, &image_create_info, NULL, &mappableImage));
    assert(res == VK_SUCCESS);
    ...
    /* allocate memory */
    res = VKCALL(vkAllocateMemory(info.device, &mem_alloc, NULL, &(mappableMemory)));
    assert(res == VK_SUCCESS);

    /* bind memory */
    res = VKCALL(vkBindImageMemory(info.device, mappableImage, mappableMemory, 0));
    assert(res == VK_SUCCESS);
    ...
    /* Queue the command buffer for execution */
    res = VKCALL(vkQueueSubmit(info.graphics_queue, 1, submit_info, cmdFence));
    assert(res == VK_SUCCESS);
    ...
    do {
        res = VKCALL(vkWaitForFences(info.device, 1, &cmdFence, VK_TRUE, FENCE_TIMEOUT));
    } while (res == VK_TIMEOUT);
    assert(res == VK_SUCCESS);

    vkDestroyFence(info.device, cmdFence, NULL);

    res = VKCALL(vkMapMemory(info.device, mappableMemory, 0, mem_reqs.size, 0, &data));
    assert(res == VK_SUCCESS);
    ...
    res = VKCALL(vkResetCommandBuffer(info.cmd, 0));
    res = VKCALL(vkBeginCommandBuffer(info.cmd, &cmd_buf_info));
    assert(res == VK_SUCCESS);
    ...
    if (!needStaging) {
        ...
        res = VKCALL(vkCreateImage(info.device, &image_create_info, NULL, &texObj.image));
        assert(res == VK_SUCCESS);
        ...
        /* allocate memory */
        res = VKCALL(vkAllocateMemory(info.device, &mem_alloc, NULL, &texObj.mem));
        assert(res == VK_SUCCESS);

        /* bind memory */
        res = VKCALL(vkBindImageMemory(info.device, texObj.image, texObj.mem, 0));
        assert(res == VK_SUCCESS);
    ...
    res = VKCALL(vkCreateImageView(info.device, &view_info, NULL, &texObj.view));
    assert(res == VK_SUCCESS);
}
...
void destroy_uniform_buffer(struct sample_info &info) {
    for (int i = 0; i < info.uniform_datas.size(); i++) {
        vkDestroyBuffer(info.device, info.uniform_datas[i].buf, NULL);
        vkFreeMemory(info.device, info.uniform_datas[i].mem, NULL);
    }
}
